...
# Is there a QEMU built and installed already, that we can potentially use?
if [[ -d "${PREFIX}/usr" ]]; then
	# Do we need to rebuild anyway (e.g., because something changed)?
...
[[ $? -eq 0 ]] || fail "configure script failed for QEMU $VER during test $TESTNAME"
...
[[ $? -eq 0 ]] || fail "Install failed for QEMU $VER during test $TESTNAME"
